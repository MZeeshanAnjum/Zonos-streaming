...
class Zonos(nn.Module):
    ...
    def _decode_one_token(
        ...
        need_capture = (
            self._cg_graph is None or self._cg_batch_size != bsz
        )
        ...
        if need_capture:
            ...
            self._cg_input_ids.copy_(input_ids)  # ← THIS LINE was wrongly indented
    ...
    def stream(
        ...
        #from zonos.utils import apply_delay_pattern, revert_delay_pattern, sample_from_logits
    
        assert cfg_scale != 1, "TODO: add support for cfg_scale=1"
        assert len(chunk_schedule) > 0, "chunk_schedule must not be empty"
        assert all(chunk_overlap < size for size in chunk_schedule), "chunk_overlap must be less than chunk size"
        ...
        with torch.device(device):
            ...
            codes = torch.full((batch_size, 9, audio_seq_len), unknown_token, device=device)
        ...
        while torch.max(remaining_steps) > 0:
            ...
            eos_codebook_idx = torch.clamp(eos_codebook_idx, max=8)
